 ...
 * modified on github.com/natetyler/wikiquotes-api
 */
 ...
 */
  wqa.queryGeneratorLinks = function (topics, success, errorFunc, isMatch) {
    ...
    // If `isMatch` flag set, query exact matching titles on the topics.
    if (!isMatch) {
      payload.generator = "link" //"linkshere",
    ...
    $.ajax({
      ...
      success: function (result, status) {
        if (!result || !result.query) {
          status = 404
          console.log(result)
          errorFunc('queryGeneratorLinks', result)
          return
        }
        var pages = Object.keys(result.query.pages)
          .filter(p => p > 0)
        if (!pages || pages.length <= 0) {
          errorFunc("No results")
          return
        }
